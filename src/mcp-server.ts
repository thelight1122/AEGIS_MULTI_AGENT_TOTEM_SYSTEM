...
server.tool("aegis_list", "List discovered lanes and Folder Totems.", { lanes: z.boolean().optional(), folders: z.boolean().optional() }, async (options) => {
  const status = await getStatus(repoRoot);
  return text(formatInventory(status, resolveInventoryScope(options)));
});
